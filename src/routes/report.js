...
// GET /filter-notifications - Fetch notifications with filters (type_id, webinar_id, status, student_id)
router.get("/filter-notifications", authenticate, async (req, res) => {
  const { type_id, webinar_id, status, student_id } = req.query;
  ...
  try {
    ...
    // Fetch status logs for the filtered notifications
